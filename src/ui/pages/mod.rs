...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for id in epics.keys().sorted() {
            let epic = &epics[id];
            let id_col = get_column_string(&id.to_string(), 18);
            let name_col = get_column_string(&epic.name, 26);
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_col = get_column_string(&self.epic_id.to_string(), 8);
        let name_col = get_column_string(&epic.name, 17);
        let description_col = get_column_string(&epic.description, 25);
        let status_col = get_column_string(&epic.status.to_string(), 20);
        ...
        for id in epic.stories.iter().sorted() {
            ...
            let status_col = get_column_string(&story.status.to_string(), 22);
            println!("{}|{}|{}", id_col, name_col, status_col);
        }
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        let db_state = self.db.read_db()?;
        let story = db_state.stories.get(&self.story_id).ok_or_else(|| anyhow!("Story not found"))?;

        println!("****************************** STORY DETAIL ******************************");
        println!("  id  |        name        |       description       |       status      ");
        let id_col = get_column_string(&self.story_id.to_string(), 6);
        let name_col = get_column_string(&story.name, 20);
        let description_col = get_column_string(&story.description, 25);
        let status_col = get_column_string(&story.status.to_string(), 19);
        println!("{}|{}|{}|{}", id_col, name_col, description_col, status_col);
...
mod tests {
    use super::*;
    use crate::{db::test_utils::MockDB, models::{Epic, Story}};
    ...
    mod home_page {
        ...
        fn handle_input_should_return_the_correct_actions() {
            ...
            let invalid_epic_id = "1000";
            let junk_input = "kdhgo2jhgt0";
            let junk_input_with_valid_prefix = "qkdhgo2jhgt0";
            let input_with_trailing_white_space = "q\n";
            ...
            assert_eq!(page.handle_input(input_with_trailing_white_space).unwrap(), None);
        }
    }
    ...
    mod epic_detail_page {
        ...
        fn draw_page_should_throw_error_for_invalid_epic_id() {
            let db = Rc::new(JiraDatabase { database: Box::new(MockDB::new()) });
            let page = EpicDetail { epic_id: 1001, db };
    ...
    mod story_detail_page {
        ...
        fn draw_page_should_throw_error_for_invalid_story_id() {
            ...
            let page = StoryDetail { epic_id, story_id: 1001, db };
            assert_eq!(page.draw_page().is_err(), true);
        }
        ...
        fn handle_input_should_return_the_correct_actions() {
            ...
            assert_eq!(page.handle_input(p).unwrap(), Some(Action::NavigateToPreviousPage));
            assert_eq!(page.handle_input(u).unwrap(), Some(Action::UpdateStoryStatus { story_id: 2 }));
            assert_eq!(page.handle_input(d).unwrap(), Some(Action::DeleteStory { epic_id: 1, story_id: 2 }));
