...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        parsed.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Epic not found"))?.stories.push(new_id);
        self.database.write_db(&parsed)?;
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        for story_id in parsed.epics.get(&epic_id).ok_or_else(|| anyhow!("Epic not found"))?.stories.iter() {
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let mut parsed = self.database.read_db()?;
        let epic = parsed.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Epic not found"))?;
        let story_index = epic.stories.iter().position(|id| id == &story_id).ok_or_else(|| anyhow!("Story not found"))?;
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        ...
        parsed.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Epic not found"))?.status = status;
        self.database.write_db(&parsed)?;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        parsed.stories.get_mut(&story_id).ok_or_else(|| anyhow!("Story not found"))?.status = status;
        self.database.write_db(&parsed)?;
...
pub trait Database {
    ...
    fn write_db(&self, db: &DBState) -> Result<()>;
}
